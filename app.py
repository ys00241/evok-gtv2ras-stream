...
====================================
Serves everything: HLS segments + Web UI + Recording + API
(No nginx static file serving needed)
"""
...
WEB_UI_DIR = Path(os.environ.get("WEB_UI_DIR", "/usr/share/nginx/html"))
...
def stream_status():
    ...
    return jsonify({"status":"ok","running":r,"hls_ready":(STREAM_DIR/"stream.m3u8").exists(),"config":stream_config,"channels":{k:v["enabled"] for k,v in channels.items()}})
...
def channel_control(name):
    if name not in channels: return jsonify({"status":"error","message":"Unknown"}),404
...
def record_start():
    ...
    quality = rc["quality"]
    res = stream_config["resolution"] if quality=="same" else ("1280x720" if quality=="720p" else "1920x1080")
    fps = stream_config["fps"] if quality=="same" else 30
...
def record_status():
    ...
    return jsonify({"status":"ok","running":r,"files":[{"name":f.name,"size_mb":round(f.stat().st_size/1048576,1)} for f in files[:20]],"disk_used_mb":sum(f.stat().st_size for f in files)/1048576 if files else 0})
